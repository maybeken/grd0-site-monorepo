<template>
    ...
    <table class="table-auto border-spacing-4 w-full">
      ...
      <tbody class="border-b">
        <tr v-for="row of getPageItem(data || [], max_item, current_page)">
          <td class="py-2" v-for="(col, key) of columns">
            {{ col.formatter ? col.formatter(row[key]) : row[key] }}
          </td>
          <td class="flex gap-4 place-content-center py-2">
            <button
              v-for="action of actions"
              class="rounded-xl border border-dotted px-4 py-px"
              @click="$emit(action.name, row[action.data_key])"
            >
              {{ action.display_name }}
            </button>
          </td>
  ...
  <div class="flex gap-4 text-sm place-content-end">
    ...
    <button
      ...
      class="disabled:hidden"
      :class="current_page == idx ? ['underline'] : []"
      :disabled="
        !(
          getNumberOfPages(data || []) <= 5 ||
          idx == 1 ||
          idx == getNumberOfPages(data || []) ||
          current_page == idx
        ) &&
        (idx > current_page + 2 || idx < current_page - 2)
      "
    >
      {{ idx }}
    </button>
...
const { columns, data, actions } = defineProps<Props>()
