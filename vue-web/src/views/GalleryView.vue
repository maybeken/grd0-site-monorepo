...
<template>
  ...
  <DropdownSelection
    :disabled="loading"
    :options="gallery_collection"
    :selected="selected_collection"
    :stylize="(val: string) => formatCollectionName(gallery_collection, val)"
    @select="selectCollection"
  >
  </DropdownSelection>
  <div
    class="py-2 relative rounded-xl"
    v-if="getCollectionCover(gallery_collection, selected_collection)"
  >
    <CDNImage
    ...
    <div class="absolute top-1/2 text-center w-full">
      <p class="px-4 md:text-2xl font-bold tracking-widest uppercase font-serif">
        {{ formatCollectionName(gallery_collection, selected_collection) }}
      </p>
    </div>
  </div>
  <div
    class="py-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-2"
  >
